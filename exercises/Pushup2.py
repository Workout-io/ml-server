...
def process_frame_pushup(frame):
    global counter, stage, correct_pushups, incorrect_pushups, screenshot_counter
    ...
    if results.pose_landmarks:
        ...
        correct_elbow = [elbow[0], elbow[1] - 0.1]  # Adjust the y value as needed

        # Draw correct position lines
        cv2.line(
            image,
            tuple(np.multiply(elbow, [640, 480]).astype(int)),
            tuple(np.multiply(correct_elbow, [640, 480]).astype(int)),
            (255, 255, 0),
            2,
        )
